The growing access to AI/ML technologies such as LLM tools (ChatGPT, etc.) has given rise to a phenomenon now known as [Vibe Coding](https://en.wikipedia.org/wiki/Vibe_coding). In simple words, it is a practice of writing code by simply using AI without understanding what it is actually doing.

A worse form of this is ChatGPT-based emails.

Instead of messages (please use GitHub Discussions for these, as mentioned in our README.md) that simply ask a question ("My concore build is failing with this error. See attached. Can you help with what went wrong?"), we end up receiving bloated essays with several irrelevant texts and questions.
...
Such questions waste the mentoring resources. While some of us are well-experienced in ChatGPT to spot these fake questions, many are not. Our mentors often end up typing detailed answers to AI Slop.

Therefore, we have decided not to accept anyone who sends obvious AI slop in GitHub discussions, emails (private emails are discouraged, except for sharing the proposal draft - even in that case, add all the mentors into a single email!), or code.
